...
- **`onComplete`: one event per call, on success and on failure**, carrying the usage, the dollar cost where known, how many providers were attempted and which one answered. Previously that meant correlating two hooks with a count you kept yourself. The failure case fires too, because a completion hook that only fired on success would describe a healthier system than the real one.
  ...
  **Correction, 2026-09-25, after this released.** As shipped it fires for `generateText` and `generateChat` only, not for `generateStructured`, `runAgent` or any of the three streamed methods, although the event's `operation` type names all nine. **Do not adopt it as your only per-call spend event in this version**: a codebase that streams or uses structured output would lose those calls from its totals with no error. Keep your existing cost tap. Found by a consumer before adopting it, tracked as `TD-LLMPORTS-ONCOMPLETE-FIRES-FOR-TWO-OF-NINE-OPERATIONS`, and being completed. The tests covered `generateText` only, which is why it passed.
