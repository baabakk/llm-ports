...
- **Severity:** High
- **Status:** Resolved 2026-08-19 in `0.1.0-alpha.31.1`, commit e5de656. `RegistryOptions.auth` accepts an `AuthBackend`; `InMemoryAuth` is the default and reproduces alpha.30 behavior exactly. 11 tests in `packages/core/tests/auth-backend.test.ts`. The alpha.30 additive-compatibility claim is corrected in the alpha.31.1 changelog entry. **One part of the original resolution path was not delivered and is now tracked separately:** the interface shipped synchronous, so it does not give multi-process deployments a shared view. See TD-LLMPORTS-AUTH-STATE-CROSS-PROCESS below.
