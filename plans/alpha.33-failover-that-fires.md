...
**Why partial coverage is safe rather than a compromise.** `defaultShouldFallback` returns `true` for `ContentBlockUnsupportedError` at `errors.ts:870`, and that is the **default** classifier, not the aggressive preset. So a chain whose first provider cannot take a PDF walks to one that can, with no consumer configuration and no code change. Partial adapter coverage degrades into routing rather than into failure, which is the whole argument for having a port.
