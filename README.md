...
>
> **Coming next**: `1.0.0-rc.36`. The alpha line ends here. The next release is a candidate for `1.0.0` carrying every remaining breaking change at once, so it can be baked once rather than trickled out: a total spend ceiling per budget scope with shared storage behind it, login state that works across processes, one model record covering what a model can do and what it costs, and tool schemas converted per attempt. The sequence is set out on [the status page](https://baabakk.github.io/llm-ports/v0-1-status).
