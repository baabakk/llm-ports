...
> **📣 Current release: `0.1.0-alpha.32`** — **`streamChat`**: one streamed turn with tool calls **surfaced, not executed**. `streamText` streams tokens but cannot carry tools; `runAgent` takes tools but owns the loop and resolves once. Realtime speech needs both at the same time, and `streamChat` is that middle. Ships with the first inbound integration, [`@llm-ports/integration-livekit`](./packages/integration-livekit), which routes a LiveKit Agents voice agent through a port so it gains fallback chains, cost ceilings, and task routing that its own LLM layer does not carry. Measured overhead: **0.036 ms** to route through the Registry, **0.33 ms** per fallback hop, against a ~680 ms conversational budget.
>
> **Previous release: `0.1.0-alpha.31.1`** — correctness and type-compatibility fixes, no new features. `RegistryOptions.auth` makes authentication state injectable via a new `AuthBackend` interface and `InMemoryAuth` default, so several Registry instances can share one view of which credentials have authenticated. Alpha.30 kept that state private per instance, which meant two registries could reach opposite verdicts on the same credential. Default behavior is unchanged. Separately, `@llm-ports/telemetry-otel`'s `Tracer` and `AttributeValue` types now unify with `@opentelemetry/api`, so adopters can drop the `unknown` cast at `createOtelSink`. Also corrects the alpha.30 notes, which claimed identical behavior for consumers who did not opt in; that was false for multi-registry consumers.
>
> **Coming next** — fixing streamed fallback for `streamText` and `streamStructured`, which cannot currently fall back at all because an async generator throws after the chain walker has already returned (`TD-LLMPORTS-STREAM-FALLBACK-NEEDS-PRIMING`, found while building this release). The durable queue lives in [docs/v0-1-status.md](./docs/v0-1-status.md#near-term-alpha-queue).
