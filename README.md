...
> **📣 Current release: `0.1.0-alpha.31.2`** — evaluation persistence and workflow tooling. Pays the scope announced for alpha.31 and displaced by the hotfix that took that number. `@llm-ports/eval` gains a **Postgres backend** (peer-dep `pg`; dedup via `ON CONFLICT DO NOTHING`, which returns the exact idempotent-write boolean in one statement) and a **workflow layer**: `aggregateScores`, `detectRegression`, `sampleEvaluations`, `runBatchJudge`, `runComparison`, and the `OperationSource` port. The package still never calls a model and still has no dependency on `@llm-ports/core`; judges and replay functions are caller-supplied, so evaluation inherits your Registry's routing, fallback, and budget gating. ClickHouse is withdrawn on verified grounds rather than deferred.
>
...
>
> **Coming next** — `streamChat`: streaming with tool calls surfaced rather than executed, so a framework that owns its own agent loop can route through this library and gain fallback chains, cost ceilings, and task routing. Plus the first inbound `integration-*` package. Planned in [`plans/alpha.32-streaming-tool-calls-and-livekit.md`](./plans/alpha.32-streaming-tool-calls-and-livekit.md). The durable queue lives in [docs/v0-1-status.md](./docs/v0-1-status.md#near-term-alpha-queue); this line only points at the top of it.
