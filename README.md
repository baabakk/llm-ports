...
> **📣 Current release: `0.1.0-alpha.34`.** **Configuration that survives an incomplete deployment.** A provider whose adapter is not registered is now skipped with a warning instead of taking the whole registry down, so a deployment holding only some API keys keeps working. Models nobody is cost-gating no longer need a price, and an unknown price is reported as unknown rather than as zero, which had been quietly shrinking spend totals. The registry can be configured with an object as well as environment variables. And `@llm-ports/core` is now a peer dependency whose errors are recognised even across two installed copies, which had silently disabled failover. Three outages that were documented to fail over now do: a provider 5xx, an empty response, and an Ollama or Google endpoint that cannot be reached. **TS-BREAKING**: `cost` is optional, and `AdapterRegistration.pricing` can be `"free"`. See [the migration page](./docs/migration/alpha-33-to-alpha-34.md).
>
...
>
> **Coming next**: `alpha.35`, a non-streaming chat call that surfaces tool calls without executing them, structured output from a JSON Schema as well as from Zod, and a contract test pinning how every adapter reports validation failures.
