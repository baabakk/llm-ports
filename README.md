...
> **📣 Current release: `0.1.0-alpha.31.1`** — correctness and type-compatibility fixes, no new features. `RegistryOptions.auth` makes authentication state injectable via a new `AuthBackend` interface and `InMemoryAuth` default, so several Registry instances can share one view of which credentials have authenticated. Alpha.30 kept that state private per instance, which meant two registries could reach opposite verdicts on the same credential. Default behavior is unchanged. Separately, `@llm-ports/telemetry-otel`'s `Tracer` and `AttributeValue` types now unify with `@opentelemetry/api`, so adopters can drop the `unknown` cast at `createOtelSink`. Also corrects the alpha.30 notes, which claimed identical behavior for consumers who did not opt in; that was false for multi-registry consumers.
>
> **Previous release: `0.1.0-alpha.31`** — per-call `operation_id` precedence. Consumers that wrap the port with `withObservabilityContext(port, { operation_id })` for one call now have their id honored by the Registry on every emitted lifecycle event, instead of the Registry always minting a fresh id. New precedence chain: `withObservabilityContext(port, { operation_id })` wins, then `Instrumentation.context.operation_id` (alpha.29 Registry-level slot), then a fresh mint. Unblocks BEPA-style capability wrappers to pre-mint an id and correlate quality-tracker records, incident-logger DB rows, and OTel spans by a shared id. Fully additive; existing callers see identical behavior.
>
> **Coming next** — the `@llm-ports/eval` persistence backends (Postgres, ClickHouse) and evaluation-workflow tooling, queued for `alpha.31.2`. Both were announced as alpha.31 scope and displaced by the hotfix above. The durable queue lives in [docs/v0-1-status.md](./docs/v0-1-status.md#near-term-alpha-queue); this line only points at the top of it.
