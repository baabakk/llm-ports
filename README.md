...
> **📣 Current release: `0.1.0-alpha.35`.** **Contract corrections, the missing chat method, and the close of the observability work.** A streamed call through an OpenAI-compatible provider reports its tokens again, so spend totals and budget gates stop under-counting for Together AI, Cerebras and their kind. A conversation whose system messages are not adjacent folds and warns where two adapters used to throw. New: `generateChat`, which returns one assistant turn with its tool calls surfaced and not executed; structured output from a JSON Schema as well as from Zod; dollar cost on OpenTelemetry spans, absent rather than zero when no price is known; a once-per-call `onComplete` hook; a bounded record of recent retries; sink composition; and an unexplained provider 400 learned once instead of rediscovered on every call. **TS-BREAKING**: `GenerateStructuredOptions.schema` is optional, which is a compile-time break for code that reads the field rather than sets it. See [the migration page](./docs/migration/alpha-34-to-alpha-35.md).
>
> **Previous release: `0.1.0-alpha.33`.** **Failover that actually fires, and documents that route.** Three fixes to the same promise. `streamText` and `streamStructured` could never fall back on any released version, because an async generator runs no body until first iteration, so the chain walker saw a healthy open for a dead provider; they now prime inside the walker. `AttemptTimeoutError` makes a blown `perAttemptTimeoutMs` walk the chain instead of surfacing, which was alpha.28's announced highest-leverage item and was never built; a cancellation you requested is never reclassified as a timeout. And `DocumentBlock` lets a PDF travel through the port at all, with uneven adapter support handled by routing rather than failure. **TS-BREAKING**: `ContentBlock` gains a member, so exhaustive switches need a new case. See [the migration page](./docs/migration/alpha-32-to-alpha-33.md).
>
> **Coming next**: `1.0.0-rc.36`, a candidate for `1.0.0` carrying every remaining breaking change at once. `0.1.0-alpha.35.1` shipped on 2026-09-25 and repaid alpha.35's one defect: `onComplete` now fires for every operation the port routes, so it is safe to use as your single per-call spend event. Take `core@0.1.0-alpha.35.1` if you rely on that hook. The next release is a candidate for `1.0.0` carrying every remaining breaking change at once, so it can be baked once rather than trickled out: a total spend ceiling per budget scope with shared storage behind it, login state that works across processes, one model record covering what a model can do and what it costs, and tool schemas converted per attempt. The sequence is set out on [the status page](https://baabakk.github.io/llm-ports/v0-1-status).
...
| `@llm-ports/eval` | Durable storage for post-hoc evaluations (LLM-judge scores, human annotations, rule-based verdicts), plus a workflow layer over them: score aggregation, regression deltas, sampling, batch judging, and A/B comparison. In-memory store (default, no deps), SQLite (peer-dep `better-sqlite3`), Postgres (peer-dep `pg`). Never calls a model itself; judges are caller-supplied, so evaluation inherits your Registry's routing and budget gating. |
| `@llm-ports/integration-livekit` | **Inbound** integration: route a LiveKit Agents voice agent through an `LLMPort`. LiveKit already abstracts providers; this adds the governance its LLM layer lacks. (alpha.32+) |
