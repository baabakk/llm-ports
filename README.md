...
> **📣 Current release: `0.1.0-alpha.33`** — **failover that actually fires, and documents that route.** Three fixes to the same promise. `streamText` and `streamStructured` could never fall back on any released version, because an async generator runs no body until first iteration, so the chain walker saw a healthy open for a dead provider; they now prime inside the walker. `AttemptTimeoutError` makes a blown `perAttemptTimeoutMs` walk the chain instead of surfacing, which was alpha.28's announced highest-leverage item and was never built; a cancellation you requested is never reclassified as a timeout. And `DocumentBlock` lets a PDF travel through the port at all, with uneven adapter support handled by routing rather than failure. **TS-BREAKING**: `ContentBlock` gains a member, so exhaustive switches need a new case. See [the migration page](./docs/migration/alpha-32-to-alpha-33.md).
>
> **Previous release: `0.1.0-alpha.32`** — **`streamChat`**: one streamed turn with tool calls **surfaced, not executed**. `streamText` streams tokens but cannot carry tools; `runAgent` takes tools but owns the loop and resolves once. Realtime speech needs both at the same time, and `streamChat` is that middle. Ships with the first inbound integration, [`@llm-ports/integration-livekit`](./packages/integration-livekit), which routes a LiveKit Agents voice agent through a port so it gains fallback chains, cost ceilings, and task routing that its own LLM layer does not carry. Measured overhead: **0.036 ms** to route through the Registry, **0.33 ms** per fallback hop, against a ~680 ms conversational budget.
>
> **Coming next** — `alpha.34`, "configuration that survives an incomplete deployment": a missing API key stops taking down every other configured provider, pricing stops being required for models nobody is cost-gating, and the Registry gains a programmatic config path alongside environment variables. Beta is gated on a list rather than a date; both the list and the release sequence are in [plans/ROAD-TO-BETA.md](./plans/ROAD-TO-BETA.md).
