...
>
> **Coming next**: a `0.1.0-alpha.35.1` dot-release, then `1.0.0-rc.36`. The dot-release repays alpha.35: `onComplete` shipped covering two of the operations its type advertises rather than all of them, so do not make it your only per-call spend event until that lands. The next release is a candidate for `1.0.0` carrying every remaining breaking change at once, so it can be baked once rather than trickled out: a total spend ceiling per budget scope with shared storage behind it, login state that works across processes, one model record covering what a model can do and what it costs, and tool schemas converted per attempt. The sequence is set out on [the status page](https://baabakk.github.io/llm-ports/v0-1-status).
