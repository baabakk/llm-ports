...
> **📣 Current release: `0.1.0-alpha.31`** — per-call `operation_id` precedence. Consumers that wrap the port with `withObservabilityContext(port, { operation_id })` for one call now have their id honored by the Registry on every emitted lifecycle event, instead of the Registry always minting a fresh id. New precedence chain: `withObservabilityContext(port, { operation_id })` wins, then `Instrumentation.context.operation_id` (alpha.29 Registry-level slot), then a fresh mint. Unblocks BEPA-style capability wrappers to pre-mint an id and correlate quality-tracker records, incident-logger DB rows, and OTel spans by a shared id. Fully additive; existing callers see identical behavior.
>
