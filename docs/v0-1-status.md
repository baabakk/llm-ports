...
| Fix streamed fallback for `streamText` / `streamStructured` | **Next, correctness** | Neither method can fall back at all on any released version: `walkStreamChain` expects `openStream` to throw, but an async generator runs no body until first iteration, so the walker sees a healthy open for a dead provider. Found while building `alpha.32`, which avoids it by priming. `primeStream` and `replayPrimed` already exist, so the fix is small. See `TD-LLMPORTS-STREAM-FALLBACK-NEEDS-PRIMING`. |
