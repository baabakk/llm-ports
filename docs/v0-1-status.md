...
| The unshipped remainder of alpha.28's slate | **Owed; inventory done 2026-08-21** | **4 of 16 shipped, 2 partial, 10 not.** Full scoring in [`plans/alpha.28-reliability-observability-polish.md`](../plans/alpha.28-reliability-observability-polish.md), verified against source rather than against changelogs. Per consumer: ADW 0 of 5, SalesCoach 1 of 5, Dramma 1 of 4, BEPA 2 of 3. The consumer report that prompted this was exactly right. Highest-value unshipped items are `AttemptTimeoutError` (the release's own stated highest-leverage item, wanted by two consumers) and the combined `onComplete` hook (also two consumers). |
| `DocumentBlock` in the content model | **New scope, queued after the owed work** | The port cannot carry a PDF: `ContentBlock` has no document member and `ImageSource` admits four raster formats. A consumer bypasses the port with a vendor SDK because of it, and its settings panel consequently shows a provider choice that does not govern half its pipeline. Recommend a separate block rather than widening `ImageSource`, following the precedent `AudioBlock` already set. See `TD-LLM-PORTS-NO-DOCUMENT-BLOCK`. |
