...
| `@llm-ports/eval` persistence backends beyond SQLite | Queued for `alpha.31.2`, displaced from alpha.31 | Stated as alpha.31 scope, displaced by the per-call `operation_id` hotfix that shipped under that number. **Postgres only.** ClickHouse cannot honor the store's exact-dedup contract: dedup happens only at unpredictable background merges, and `FINAL` is documented as eventual-correctness-only and explicitly not to be relied on. `insert_deduplication_token` fits but is window-bounded and still cannot give `write()`'s exact boolean. See `TD-LLMPORTS-ALPHA31-EVAL-BACKENDS-DEFERRED`. |
| Evaluation-workflow tooling | **Dropped pending a definition** | No design document defines this beyond the original README phrase. Rather than invent scope from four words, it comes off the queue until someone states what it is. |
...
**Ordering note.** The first two rows carry no new public API and address better-evidenced demand than the streaming surface does, which is why they sit above it despite being smaller. The `alpha.32` work is scheduled ahead of the queued `alpha.31.2` eval backends because it has a waiting consumer; the version numbers are labels and do not imply a dependency.
