...
| Evaluation-workflow tooling | **Proposed for removal, needs a decision** | No design document defines this beyond the original README phrase. Recommendation is to drop it rather than invent scope from four words, but it was publicly announced, so removing it is an owner's call and not a maintainer's. |
