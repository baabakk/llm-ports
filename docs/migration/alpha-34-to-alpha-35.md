...
- **`onComplete`** fires once per call, on success **and on failure**, with usage, dollar cost where known, how many providers were attempted and which answered. **Incomplete in this version and fixed in `0.1.0-alpha.35.1`:** as shipped here it covers `generateText` and `generateChat` only, so take the dot-release before making it your only spend event. See `TD-LLMPORTS-ONCOMPLETE-FIRES-FOR-TWO-OF-NINE-OPERATIONS`.
